...
| 文档转换 | MarkItDown | 统一 PDF、Office、CSV/JSON/XML、HTML、EPUB、ZIP、MSG、图片、音频等格式到 Markdown |
| 章节拆分 | `scripts/mdsplit_obsidian.py` | 已针对中文教材章节标题做过适配 |
| LLM | DeepSeek 离线导出 + OpenAI-compatible API | 优先读取 `deepseek_keyword_cache/*.json` 写入图谱节点；无离线导出时可调用 API 补全标签、层级和证据线索 |
...
3. Markdown/TXT 直接读取；PDF、Office、表格、HTML、EPUB、ZIP、MSG、图片和音频等格式调用 MarkItDown 转换。
...
| 依赖 | 正常路径 | 降级路径 |
| --- | --- | --- |
| MarkItDown Python 包 | 直接转换 PDF、Office、HTML、表格、EPUB、ZIP、图片/音频等格式 | 查找 CLI；仍失败则返回明确错误 |
| DeepSeek | 优先读取离线导出，或调用 API 主提取关键词/知识点并写入图谱节点 | 回退本地规则图谱，不阻塞核心流程 |
